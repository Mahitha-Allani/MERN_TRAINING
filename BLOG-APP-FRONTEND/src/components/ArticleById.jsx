...
// ArticleByID component displays a single article with its details and comments.
...
// Toggles the article's active status (delete/restore) for authors.
...
// Navigates to the edit article page, passing the current article data in state.
  const editArticle = () => {
    navigate(`/edit-article/${article._id}`, { state: article });
  };
// Adds a new comment to the article and updates the comments list.
  const addComment = async () => {
    if (!comment.trim()) return;
    try {
      const res = await axios.post(
        `${BASE_URL}/user-api/articles/${article._id}/comments`,  
        { comment },
        { withCredentials: true }
      );
...
// Deletes a comment by its ID and updates the comments list.
  const deleteComment = async (commentId) => {
    try {
      await axios.delete(
        `${BASE_URL}/author-api/comments/${commentId}`,  
        { withCredentials: true }
      );
...
// Determines if the article is deleted based on its active status.
